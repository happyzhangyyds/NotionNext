...
export async function getGlobalData({ pageId = BLOG.NOTION_PAGE_ID, from }) {
  ...
  if (db?.notice) db.notice = cleanBlock(db?.notice)
  if (db?.post) db.post = cleanBlock(db?.post)
...
function getLatestPosts({ allPages, from, latestPostCount }) {
  const allPosts = allPages?.filter(p => p.type === 'Post' && p.status === 'Published')
...
function getSiteInfo({ collection, block }) {
  ...
  const description = collection?.description ? collection.description[0][0] : BLOG.DESCRIPTION
...
async function getDataBaseInfoByNotionAPI({ pageId, from }) {
  ...
  for (let i = 0; i < pageIds.length; i++) {
    ...
    if (!value) {
      ...
      if (pageBlock.block[id].value) {
        const properties = await getPageProperties(id, pageBlock.block[id].value, schema, null, getTagOptions(schema))
        if (properties) {
    ...
    const properties = await getPageProperties(id, value, schema, null, getTagOptions(schema))
    if (properties) {
      collectionData.push(properties)
    }
  }
  ...
  const allPages = collectionData.filter(post => {
    ...
    return post && post?.slug && (!post?.slug?.startsWith('http')) && (post?.status === 'Invisible' || post?.status === 'Published')
  })
